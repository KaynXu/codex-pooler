...
2. **Allow up to** permits only known efforts through the selected ceiling: `none`, `minimal`, `low`, `medium`, `high`, `xhigh`, `max`, or `ultra`. The Pool model's known levels (the union across the Pool's assignments) can narrow the result further; the assignment that finally serves may list fewer, in which case an explicit effort it does not list is the upstream's to refuse. An omitted request uses the permitted model default, or the highest permitted known level.
