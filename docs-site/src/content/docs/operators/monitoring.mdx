...
Like the saved-reset metric, this one is best-effort per role. Worker and scheduler roles do not run the Prometheus reporter; on a split deployment the `stale_sweep` phase is emitted there and reaches the app reporter through the telemetry relay under `via="job_relay"`, best effort and at most once. The durable record is authoritative in every topology: every reservation-failure release entry carries a `pre_attempt_phase` value in its `details`, and an entry with no such key is either a settlement-time release or older than the field.
