...
Codex retries `429`, `5xx` and transport errors at its HTTP client and decides
stream and turn retries from its own error variant, not from the type. Keep that behaviour and use the type and `error.code` for diagnosis,
...
API-key policy denials that Codex Pooler authors (`api_key_missing`,
`api_key_disabled`, `api_key_policy_malformed`, `model_not_allowed`) keep their
